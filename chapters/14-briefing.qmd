...
![**a** — every year-to-year transition in the national panel, placed by how much the mean moved and by how far the top band moved away from what that change of mean alone predicts. Euskadi's are violet; 2024 and 2025 are ringed, each with a leader to its label. The dashed horizontal is the locus; the vertical axis is the residual from it. 170 transitions — 187 community-years less each community's first year, 160 of them outside Euskadi. **b** — those two Basque years, observed against the prediction, for both published band quantities. **c** — the 2025 → 2026 fall split into the part common to the nine reporting communities, the two Basque-specific parts that can be quantified, and the part that cannot.](../plots/fig25_decomposition.png){#fig-b-decomp}
...
| H2 The 2026 Basque paper **as a format** — length, optionality, genre | Consistent with everything; first-order variable | Set apart from H1b: reading load $+39$ %, 1 026 to 1 431 words of statement ($+45$ % on the whole-paper count, 1 335 to 1 940); choice halved, two of four problems carrying none ($-0.26$ by simulation); the only paper in the set that changed genre. The whole distribution displaced; damage confined to Física and Matemáticas II; the Catalan mirror image. |
...
3. **Keep a monitoring set**: mean, pass rate, band shares, and each subject's mean against its own 2015–2019 norm, computed per tribunal. **And write down the 2027 expectation before the sitting, because the panel will supply a recovery whether or not anything is fixed.** Within a community, a deviation from the field retains only about a fifth of itself the following year ($\phi = 0.186$). Euskadi's 2026 deviation from its own norm is $-2.16$; if that is a transient shock the 2027 figure should be near $-0.40$ (95 % $[-1.32, +0.52]$), and if it is a permanent level change it should stay near $-2.16$ ($[-3.08, -1.24]$). The two are 3.7 $\sigma$ apart and one sitting decides between them. **A recovery of about a mark and a half in 2027 is what doing nothing predicts, and is not evidence that any change worked** ([the reversion section](06-interpretation.qmd#mean-reversion)). The subject-against-own-norm comparison is what showed that Física alone was below norm in 2025; the band shares are what would have shown the 2024 compression at the time. *Evidence: strong — both findings came from these statistics.*
...
7. **Pace the deployment: change one axis a year and announce it.** The one paper that moved on four axes at once fell furthest; the 2025 break taken alone was absorbed better than the Spanish average. There is now one outside case with a pace to compare against: Cataluña tripled its examined-reasoning share at a single step in **2020** and then left the paper alone for five years, and its position against the field moved $-0.85$ on a design that could not have resolved anything below $1.26$ — most of which is other communities improving rather than Cataluña falling. *Evidence: n = 1 — and the count of axes is four on this study's own coding, three on an earlier one — which is why it sits seventh rather than first despite being the most plausible recommendation in the list. 2028 is the year the model is to be fully deployed.*
