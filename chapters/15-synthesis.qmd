...
Three things have to be said with that. **It is sufficiency, not proof**: those four figures were fitted, not predicted. **The deduction count is the solid number** — twelve in every variant of the model tried — while the cascade shares are an order of magnitude rather than an estimate. And **the model fails on the one figure it was not fitted to**: about 2.1 % of the 2026 scripts scored nine or above, and twelve deductions spread evenly cap a paper at about 8.8, so the model leaves almost nobody there. The real regime cannot have hit the best work as hard as the rest — which the tally can check directly by plotting deductions per script against the script's mark.

The three numbers that would settle this are on the marking sheets: deductions per script, the share of scripts with a voided apartado, and how much of those scripts was voided.
