...
#### Why it fails: fatigue is a level effect

The deeper reason is structural rather than empirical. Física has been the last quantitative
paper for years. Whatever fatigue costs a candidate, it cost the same in 2025 as in 2026, and
it differences out of the $-1.48$ exactly as a national shock differences out of the field
comparison in [the decomposition](#what-the-decay-is-made-of). Fatigue is in the *level* of
the Física mean relative to its neighbours — a real quantity, and one that bears on the 2027
design discussion — but a constant cannot produce a change.

There is one fact that would convert it into a change effect, and it is cheap to obtain: the
2025 and 2026 calendars, side by side. Day, slot, number of papers between Química and
Física, and the gap between days are all published by the coordination for both years. If the
slot moved, fatigue enters [the mark budget](#the-mark-budget) with a line of its own; if it
did not, it belongs in the design chapter and nowhere else. Neither sentence should be
written before the two calendars have been compared, and this dossier holds neither.
...
#### What the simulation would be, and what it would yield

The calculation is well posed. It needs the UPV/EHU ponderación table (which degrees weight
Física at 0.2, at 0.1, at 0), the cut-off marks for those degrees, and a joint distribution
of candidates' marks in the other subjects, which the published subject distributions supply.

What it yields is **a bound, not a measurement**: the fraction of candidates whose best two
weighted marks are already settled before Física is sat, and for whom the paper's marginal
contribution to the admission score is therefore zero. That fraction is the largest the
effect can possibly be. If it is three per cent of the cohort the hypothesis is finished; if
it is a quarter of the top band it accounts for 2024 unaided. The bound is obtainable from
published tables and is honest in a way that any statement about what candidates actually did
would not be, because no published aggregate records effort.
