...
**H1c — "The syllabus was reopened, and the schools were prepared for a different paper."** *Plausible, unmeasured, and not separable from H1b.* The narrowing recorded in [Sixteen years of one paper](#sec-ch12) was *de facto* and not documentary: no text ever excluded mirrors, standing waves or nuclear decay; they were simply never set, and fifteen years of never being set is a stronger signal to a school than any syllabus. 2026 reopened them, and withdrew the choice in the same sitting. The chapter also fixes the channel and the bound: in June 2026 the new sub-topics sat in **optional** slots, so the mechanism is **preparation** — schools covering the modern-physics block in full for the first time, and blocks C and D in a form that made the option unpredictable — and not an unavoidable item. That bounds it, and makes option take-up the measurement, which is data request 5. This is a real mechanism and it is separate in principle from the competency conversion, but not in the data: both are properties of the same paper in the same year, and both would displace the whole distribution downward. No published aggregate distinguishes them. Sub-item marks would.
