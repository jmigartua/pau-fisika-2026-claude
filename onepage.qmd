...
against the field over 2015–2025 ([the event study](#catalunya-event)),
...
### The route, not the destination {#competency-path}

The scatter panels above plot one year against one change, and a scatter cannot show a
*route*. Two systems can arrive at similar competency content having come from opposite
directions and from different distances, and after the Catalan archive that distinction is
the question rather than a refinement of it. Figure @fig-competency-path therefore draws
each community as an arrow from the competency-coded share of its 2025 paper to the share
of its 2026 paper, against the change in its mean — one measure throughout, so the
horizontal axis means the same thing for every arrow.

![Each community as an arrow from (competency-coded share of the 2025 paper, 0) to (share of the 2026 paper, change in the ordinary-sitting Física mean). Red = Euskadi; violet = Cataluña; orange = fell; blue = rose. Six communities have no horizontal component: they were at zero in both years. The Catalan note records what the arrow cannot — its 2025 position was a steady state held since at least 2020, so its leftward arrow is a retreat from a mature practice, not the reversal of a conversion.](plots/fig36_competency_path.png){#fig-competency-path}

Three things are visible here that the scatter panels cannot show. Euskadi's arrow is the
only one that runs far to the right and steeply down, 25 to 62.5 per cent. Cataluña's runs
*left* and slightly up, 75 to 37.5, from a position it had occupied for at least six years.
And six arrows are vertical at $x=0$: communities whose papers contained no competency-coded
marks in either year, spanning $-0.83$ to $+1.13$ in outcome.

That vertical spread at zero is the figure's real content, and it is a bound rather than a
correlation. Whatever moved those six communities by up to 1.9 marks between them was not
competency conversion, because they did none. Any account of 2026 that rests on the
conversion alone must explain why the same year moved papers that never converted by as
much as it moved the one that did.
...
### Cataluña: a steady state read as a conversion {#catalunya-history}

Cataluña's bar in @fig-profiles is the one that misleads, and it misled this study
first. Read across the row it looks like a community that converted in 2025 and
pulled back in 2026. The ordinary papers say otherwise.

{{< include data/tables/catalunya_history.md >}}

Real settings and explicit demands to justify, reason or evaluate appear in every
year from 2020, five before the decree: a singer's voice and the Apollo 15 feather
(2020), Tycho Brahe (2021), an eel's locomotion and a launch (2022), the moons of
Mars and the press coverage of a fusion announcement (2023), a planetary mission and
the altitude of a firework (2024). What 2025 changed was the structure — four block
exercises with an internal option — not the substance.

So Cataluña's 75 % in 2025 is a steady state. It is a system that has examined this
way for years, sitting about $+0.65$ above the field in both 2025 and 2026, once with
three quarters of its marks competency-coded and once with half. That is the
strongest evidence in this dossier against the proposition that competency examining
depresses marks, and it moves the weight onto the transition rather than the
paradigm.

The coding of 2020–2024 was done once, by one reader, after the 2026 result was
known; 2025 and 2026 were coded blind by two ($\kappa = 0.76$). The series is
evidence of a direction of travel, not a measurement of equal standing.

### How far back the steady state goes, and what it cost {#catalunya-event}

The previous section could say that Cataluña's 2025 paper was not a conversion. It
could not say when the conversion had been, because the archive stopped at 2020. The
June papers from 2005 are on an open archive, and fifteen of them have been added
([the registry](#sec-ch08)). With them the question becomes answerable, and it
splits in two: *when did the Catalan paper change*, and *what did the change cost*.

#### The change is datable, and it is a step at 2020

One mechanical rule was applied to every Catalan paper from 2010: what share of its
items require the candidate to justify, reason, explain, evaluate, interpret or say
why, as against compute, state or describe. The rule is a verb list, it does not know
which year it is reading, and it is reproducible
(`scripts/35_demand_history.py`).

| | mean | sd | range |
|---|---|---|---|
| 2010–2019 | **17.7 %** | 10.9 | 6.2 – 33.3 |
| 2020–2023 | **50.0 %** | — | 37.5 – 62.5 |

: The Catalan paper on one mechanical measure. 2024 falls back to 14.3 % on a paper whose structure changed to four-of-seven; 2025 returns to 66.7 %. {#tbl-cat-demand}

The level roughly triples and the step is at 2020 — five years before the decree and
five years before the structural change that made the 2025 paper look converted. The
series is noisy year to year, and no weight is put on any single year.

**The number that matters most here is the floor, not the step.** Cataluña's lowest
year across the whole traditional decade is **6.2 %**, and the mean is 17.7 %. It
never set a paper that asked nobody to justify anything. Set against the Basque
instrument of the same decade — forty per cent of the marks from a closed list of
twenty-two recall titles, and competency-carried marks at zero as late as 2024
([Sixteen years of one paper](#inertia)) — the contrast is
not one of degree. Cataluña intensified a practice it already had. Euskadi introduced
one it did not have. The two measures are different and are not compared as numbers;
the statement is qualitative and is meant as such.

#### The cost is bounded, not measured

![**a** — the Catalan paper on one mechanical measure, 2010–2025; grey before the 2020 step, violet after. **b** — Cataluña's mean minus the fifteen-community field, with the COVID sittings excluded because they confound the treatment with the control. The power statement is printed on the panel so the $-0.85$ cannot be read without it.](plots/fig37_catalunya_event.png){#fig-cat-event}

Differencing against the fifteen communities with a complete 2015–2025 Ministry
series, and dropping 2020 and 2021 — every community's mean rose in those sittings,
and they are also the first two years of the new Catalan style, so they confound what
is being measured with what is controlling for it:
...
: The Catalan position against the field, before and after the 2020 step. File: `data/analysis/catalunya_event.json`. {#tbl-cat-event}

Welch $p = 0.087$; an exact permutation test over all 126 splits of the nine
non-COVID years gives $p = 0.119$; dropping 2018, the pre-period's outlier, gives
$-0.60$ ($p = 0.19$); on medians it gives $-1.27$. **And the smallest shift this
design can detect at 80 % power is 1.26 marks.** The estimate is smaller than the
resolution of the instrument that produced it. It is a bound and not a coefficient,
and it is reported that way here because the temptation to quote it as one is
obvious.

Two further facts weaken it. **Most of the shift is the field rising, not Cataluña
falling**: Cataluña's own mean fell 0.34 while the field rose 0.50, so 59 % of the
gap change belongs to the other communities. And Cataluña's position against the
field is restless in the absence of any change at all — the year-to-year standard
deviation is 0.71, and in 2017 it moved $+1.28$ in a single year with nothing
happening to its paper, which is half again the size of the entire post-conversion
shift.

#### What this licenses, and what it does not

It does not license a coefficient. "Cataluña shows that conversion costs 0.85 marks"
would be quoting a figure the design cannot resolve, drawn mostly from other
communities improving. No such sentence appears in this dossier.

It licenses two statements. First, a system that tripled its examined-reasoning share
at a single step produced nothing a well-specified event study could detect across
the six years that followed, while Euskadi's second-year fall alone is $-1.65$
against the same kind of field — larger than the whole Catalan post-conversion
displacement, and larger than the threshold that design would have needed to see one.
The destination is not what hurt. That was already this dossier's reading; it is the
first evidence for it from outside the 2026 cross-section.

Second, and this is the one that carries: **Cataluña changed once, from a floor of
6.2 %, and then left the paper alone for five years** while its contextualisation
drifted upward on its own. Euskadi went 0 to 25 to 62.5 per cent in two sittings and,
in the same two sittings, halved optionality, reopened the syllabus and changed the
marking regime. The Catalan archive cannot tell us what a conversion costs. It tells
us what a conversion looks like when it is not also four other things at once.

#### One measure that was built and discarded

The same mechanical rule was applied to the Basque archive, so that both systems
would sit on one measure. It does not work, and the failure is recorded rather than
hidden. The splitter returns 7 to 12 items for Basque years known to offer eight
(`data/ehu_formats.csv`); some years yield only the theory titles; and one 2024
problem was counted twice because a degree symbol renders two ways in the text layer.
An early version of the measure, before nouns were excluded from the verb list, put
the 2024 Basque paper at 37.5 % against 12.5 % the year before — which, had it been
believed, would have manufactured an instrument change in the one year this dossier
says the instrument did not move, and handed [the 2024 shape
change](#a-basque-signal-that-arrives-a-year-early) a mechanism
that does not exist. The Basque series was dropped; the script records why.

### The field retreated in the second year

Across the other eight communities the competency-coded share of marks **fell** from
11.2 % to 7.2 % between 2025 and 2026. Cataluña halved its own while holding total
substantive context at 87.5 %, keeping the context and dropping the demand;
Comunitat Valenciana went to zero; five were at zero and stayed. The field against
which the Basque-specific component is measured was moving away from the competency
format while Euskadi moved towards it.
...
## What the predictability built — and what two courses of notice could not undo {#inertia}

The two preceding sections describe an instrument. Put together, they describe an
incentive, and the incentive is what a school responds to.

For fifteen years the paper made two promises. **Forty per cent of the marks came
from a closed list of titles**, and the inventory of what was actually set is
narrower than the list: 120 theory slots across 2010–2024 drew on **22 distinct
titles**, a mean recurrence of 5.5, the commonest set twelve times. And
**optionality was 100 %** — in the two-option format a candidate chose a whole
paper, and in the four-of-eight format any two problems and any two questions.
Every single item could be declined.

A teacher optimising for their students' results under that instrument would teach
the twenty-two titles thoroughly, drill the problem templates that recur, and let
the rest go. That is not neglect; it is the correct response to the incentive, and
any system that holds teachers accountable for results produces it. **The exam was
the curriculum**, and the *de facto* narrowing of the previous section is what the
exam wrote.

The clearest case is modern physics. Fourteen of the fifteen modern-physics problems
set between 2010 and 2024 were photoelectric. Fission and fusion existed **only as
theory titles** — examinable as recited prose, never as a calculation. In 2026
carbon-14 dating and plutonium-239 decay appear as problems. A student who had
learned nuclear physics exactly as the exam had asked for it, for fifteen years, met
it as arithmetic.

### The schools were told, and told in time

This must be stated plainly, because the obvious inference from the paragraphs above
is the wrong one. The reopening was **announced in 2024**, and the two academic
years **2024–25 and 2025–26 were designated transitional** by the coordination
precisely so that schools could prepare for a complete syllabus in 2026. The schools
knew. They were given two courses.

So the gap is not an information failure, and nothing in this dossier should be read
as one. What took longer than two years was not the knowledge but the **practice**.
Announcing that a topic is examinable does not by itself create worked-example
banks, mock papers, a sequence for the year that leaves room for it, or the
teacher's own fluency in setting and marking that kind of question. Fifteen years of
materials and habits were tuned to an instrument that no longer exists, and two
cohorts is not long enough to retune them.

That distinction — informed in time, and still not practised — is the honest form of
the inertia argument, and it is the form that survives contact with the record.

### And yet in June 2026 the old route was still open

There is a further fact about the 2026 papers that cuts against the obvious form of
this argument, and it has to be stated before the argument is used.

![The 2026 papers against sixteen years of their own precedent. In June the two
slots that offered **no optionality** were set on sub-topics ranked 1st and 11th of 15
by frequency, and the two sub-topics never set before — standing waves and
radioactive decay — both sat **where a choice was offered**, each beside an
alternative that had been set 10 and 14 times. July is the counter-case: its second
slot offered no choice and was a mass spectrometer, never set before.](plots/fig35_route_2026.png){#fig-route}

The June 2026 sitting — the one that produced 3.99 — contained no unavoidable
novelty at all:
...
: June 2026 slot by slot, against its own precedent. {#tbl-route tbl-colwidths="[8,18,50,24]"}

A candidate could answer **1, 2, 3b and 4a** — four sub-topics set **47 times**
between them in the preceding fifteen years, including the single most frequent
sub-topic in the entire archive in the opening slot, where no choice was offered. Both novelties sat in slots
that offered a choice, with heavily drilled alternatives beside them. **The reopened
syllabus was, in June, entirely avoidable.**

And the mean still fell 1.48.

That has a consequence which must be carried through the rest of this dossier.
**Meeting unfamiliar content cannot be what happened in June 2026**, because a
student prepared exactly as fifteen years of this exam had taught them to prepare
could have sat the whole paper on familiar ground. The content channel of the
syllabus hypothesis is therefore not merely bounded; for the ordinary sitting it is
close to empty.

What could *not* be avoided is a short list: the **genre** of the two slots with no
optionality, both competencial; the marking regime in its second year; the
reading load; and the disappearance of the theory block, four points of the ten
that had been guaranteed by preparation.

So the inertia does not disappear — **it relocates**, and into two channels that are
sharper than "they met what they had not seen".

The first is **preparation spread thinner**. Schools had to cover the whole syllabus
precisely *because* they could not know what would be set. A teaching year is finite.
Covering block E in full, and the un-set corners of blocks C and D, is time taken
from consolidating orbits, induction, lenses and the photoelectric effect. The
student who took the old route in June 2026 was therefore less drilled on that route
than a 2024 student had been — not because the route changed, but because the year
had to cover more ground to reach it.

The second is **familiar physics in an unfamiliar form**. Slot 1 was a circular-orbit
problem, the most-set sub-topic in the archive, presented as a Starlink headline to
be read, judged and explained — and with no alternative to turn to. Fifteen years had trained the recognition of a problem
type, not the reading of a real-world scenario built on one. On this reading the
inertia bites hardest exactly where the content did not change at all.

### What would now separate them

These two relocations make a prediction the content version did not, and it is
testable within the June paper itself. Slot 4 offered the photoelectric effect
(set 14 times) against carbon-14 dating (never set). Two quantities settle it:

1. **How many candidates took each option.** If they overwhelmingly took 4a, the
   novelty was avoided in fact and not merely in principle.
2. **How they scored on 4a.** If candidates did worse on the most standardised item
   in the archive than their 2024 and 2025 counterparts did, content novelty is
   excluded by the paper's own internal evidence, and preparation, genre and marking
   must carry the fall between them.

That is the smallest measurement identified anywhere in this dossier and it bears on
the largest question: one option take-up and the sub-item marks of one problem, in
three years.

### Why this is the mechanism Cataluña lacks

[Chapter 11](#catalunya-history) shows that Cataluña has set
papers with real settings and explicit justification demands in every year since
2020, on an all-problems format with optionality around half and no closed theory
list at all. There was therefore no accumulated equilibrium to unlearn. When the
decree arrived, Catalan teaching had nothing to undo; Basque teaching had fifteen
years of it, and two courses in which to undo it.

Notice what that comparison does *not* say. It does not say the Basque schools were
worse prepared in any sense they could have controlled. It says the two systems
entered the same reform carrying different amounts of accumulated practice, and that
the quantity which mattered was not notice but practice.

### What would test it

Inertia of this kind leaves the same trace in a mean as paper difficulty, marking
severity and reading load, so it is unidentified on published aggregates exactly as
[the budget](#sec-ch06) says. But unlike the others it makes a
prediction about *where* in the paper the marks went: **the loss should be
concentrated in the sub-tasks that examine what had never been set as a problem** —
modern physics beyond the photoelectric effect, standing waves, mirrors, decibels.
Marks at sub-task level would test that directly, and it is a sharper test than the
general one, because the competing hypotheses do not predict a topic pattern.
...
## The eighth round (22 September): a claim asserted from assumption {#eighth-round}

The seven rounds above caught numbers, stale fits, mis-specified baselines and one paragraph
that asserted its conclusion without its mechanism. This round caught something the earlier
rounds were not built to catch, and it is the more dangerous kind.

Writing the section on fatigue and the examination week, the dossier argued — correctly in
form — that a constant cannot produce a change, and then stated as fact that *"the order of
the papers did not change between the two years, so whatever fatigue costs is common to both
and differences out."* The logic was sound. The premise was never checked. It was inferred
from the general knowledge that Física has been sat late for years, and written in the
register of a finding.

**The premise was false.** The two published EHU timetables differ, and they differ in the
slot that the argument was about. Matemáticas II moved out of the Wednesday morning slot
(11:15–12:45) into the early-afternoon slot (14:30–16:00), which made it the paper
immediately before Física rather than a paper three hours and one other exam away. Química
moved later in its day. Física's own slot did not move, so the conclusion the false premise
supported — that Física's position cannot explain the $-1.48$ — happens to survive on other
grounds. That is luck, not method.

What the correction cost, and what it bought. It cost a section rewritten and a project
record reissued the same day. It bought the observation the false premise had foreclosed:
that in 2026 the two heaviest quantitative papers were sat back to back for the first time,
and that the four subject changes partition along exactly that pairing. The dossier now
carries that as a documented change in delivery with a matching partition, explicitly
confounded with difficulty and explicitly excluded from the budget
([the comparison](#timetable-2025-2026)).

Two general lessons are worth recording because they are not the same as the earlier ones.

**A premise stated in the voice of a finding is not audited by a numerical check.** Every
mechanical verification this dossier runs — recomputation from source CSVs, second coding,
figure regeneration — operates on quantities. "The timetable did not change" is not a
quantity; it entered the text as background, and nothing in the apparatus was looking at it.
The audit procedure now includes a specific pass: *every factual premise that an argument
turns on must name its source, or be marked as unchecked in the text.*

**Checking was cheap.** Both calendars are published by the university whose exam this is.
The elapsed cost of establishing the truth was two web fetches. The premise was asserted
rather than checked not because checking was hard but because the claim felt obvious, which
is precisely the condition under which this failure mode operates.

One point is left open rather than resolved, and is marked as such in the text: the two
sources for the 2025 grid agree on Química and Matemáticas II and disagree on whether
Biología and Física were sat on the Wednesday or the Thursday. The provisional grid in the
25/26 access guide says Thursday; the timetable published days before the sitting says
Wednesday. The direction of every statement holds either way; the magnitude does not.
...
- **Whether the front matter should be rebuilt rather than appended to.** It now carries nineteen findings accumulated over five days, and the last four were added two days after they were established, in breach of a standing instruction that it reflect every revision. Appending has kept it honest and made it long. A rebuilt front matter organised by question rather than by chronology would read better and would lose the record of what was learned when; the audit has no view on which matters more here.
